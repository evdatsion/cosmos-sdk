...
+++ https://github.com/evdatsion/cosmos-sdk/blob/v0.40.0-rc3/baseapp/baseapp.go#L46-L131
...
- [`TxDecoder`](https://godoc.org/github.com/evdatsion/cosmos-sdk/types#TxDecoder): It is used to decode
...
possibility to provide additional [`options`](https://github.com/evdatsion/cosmos-sdk/blob/v0.40.0-rc3/baseapp/options.go)
...
The [default `msgServiceRouter` included in `BaseApp`](https://github.com/evdatsion/cosmos-sdk/blob/v0.40.0-rc3/baseapp/msg_service_router.go) is stateless. However, some applications may want to make use of more stateful routing mechanisms such as allowing governance to disable certain routes or point them to new modules for upgrade purposes. For this reason, the `sdk.Context` is also passed into each [route handler inside `msgServiceRouter`](https://github.com/evdatsion/cosmos-sdk/blob/v0.40.0-rc3/baseapp/msg_service_router.go#L31-L32). For a stateless router that doesn't want to make use of this, you can just ignore the `ctx`.
...
- `GasUsed (int64)`: Amount of gas consumed by transaction. During `CheckTx`, this value is computed by multiplying the standard cost of a transaction byte by the size of the raw transaction. Next is an example:
  +++ https://github.com/evdatsion/cosmos-sdk/blob/7d7821b9af132b0f6131640195326aa02b6751db/x/auth/ante/basic.go#L104
...
+++ https://github.com/evdatsion/cosmos-sdk/blob/v0.40.0-rc3/baseapp/baseapp.go#L623-L630
...
+++ https://github.com/evdatsion/cosmos-sdk/blob/v0.40.0-rc3/types/handler.go#L6-L8
...
`BaseApp` holds an `anteHandler` as paraemter, which is initialized in the [application's constructor](../basics/app-anatomy.md#application-constructor). The most widely used `anteHandler` today is that of the [`auth` module](https://github.com/evdatsion/cosmos-sdk/blob/master/x/auth/ante/ante.go).
...
- Initialize [`deliverState`](#volatile-states) with the latest header using the `req abci.RequestBeginBlock` passed as parameter via the `setDeliverState` function.
  +++ https://github.com/evdatsion/cosmos-sdk/blob/7d7821b9af132b0f6131640195326aa02b6751db/baseapp/baseapp.go#L387-L397
